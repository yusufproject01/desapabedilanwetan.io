...
export default function CardMenuQA({
  MenuOptions,
  NavigationLink,
  Icons,
  animationsDelay,
  ...
  Icons?: any;
  animationsDelay: string;
}) {
  return (
    <Link
      href={NavigationLink}
      style={{ animationDelay: animationsDelay }}
      className={`sm:w-[8rem] sm:h-[8rem] w-[5rem] h-[7rem] bg-white border border-slate-200 shadow-lg rounded-lg flex flex-col items-center justify-center
            hover:scale-105 transition duration-300 ease-in-out hover:brightness-95 px-2 animate__bounceInUp animate__animated animate__slow`}
    >
      ...
      <div className="w-full h-1/3 flex items-center justify-center p-2">
        <h1
          className={`${inter.className} sm:text-sm text-[10px] sm:font-normal font-semibold flex items-center justify-center`}
        >
          {MenuOptions}
        </h1>
      </div>
